...
-- Enhanced return rental system - works when near depot area
local function isNearDepot()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local depotPos = Config.Depot.coords
    local distance = #(pos - vector3(depotPos.x, depotPos.y, depotPos.z))
    return distance <= 25.0 -- 25 meter radius around depot
end

-- Return rental vehicle command (enhanced to work near depot)
RegisterCommand('returnrental', function(source, args, rawCommand)
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsIn(ped, false)
    
    -- Check if player is near depot area
    if not isNearDepot() then
        QBCore.Functions.Notify('You must be near the depot to return a rental vehicle', 'error')
        return
    end
    
    if veh == 0 then
        -- Allow return even if not in vehicle, as long as near depot
        ...
        for _, vehicle in ipairs(vehicles) do
            ...
            if dist <= 10.0 then
                table.insert(nearbyVehicles, {vehicle = vehicle, distance = dist})
            end
        end
        
        -- Sort by distance and try the closest vehicle
        table.sort(nearbyVehicles, function(a, b) return a.distance < b.distance end)
        
        if #nearbyVehicles > 0 then
            veh = nearbyVehicles[1].vehicle
        else
            QBCore.Functions.Notify('No rental vehicle found nearby', 'error')
            return
        end
    end
...
local function HandleTrailerDelivery()
    ...
    -- Draw yellow down arrow marker instead of green circle
    if dist < 50.0 then
        -- Yellow down arrow marker (type 2 = down arrow, yellow color)
        DrawMarker(2, dest.x, dest.y, dest.z + 2.0, 0.0, 0.0, 0.0, 180.0, 0.0, 0.0, 1.5, 1.5, 1.0, 255, 255, 0, 200, false, true, 2, false, nil, nil, false)
